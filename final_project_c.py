...
num_epochs = 25
# hint: when u compare between the epoches look at the recall of class 4 make sure it get 0.3 or 0.2 also keep an eye on acc not only the kappa
...
def evaluate_model(model, test_loader, device, test_only=False, prediction_path='./test_predictions.csv'):
    ...
    if test_only:
        ...
        # test_kappa, test_accuracy, test_precision, test_recall = metrics[:4]
        # print(f'[test] Kappa: {test_kappa:.4f} Accuracy: {test_accuracy:.4f} '
...
class selfattention(nn.Module):
    def __init__(self, in_dim):  # Change _init_ to __init__
    ...
    def forward(self, x):
# x is expected to be a 2D tensor of shape (batch_size, features)
        batch_size, features = x.size()
        x = x.unsqueeze(1)
        # Apply linear transformations for query, key, and value
        query = self.query(x)  # Shape: (batch_size, features//8)
        key = self.key(x)      # Shape: (batch_size, features//8)
        value = self.value(x)  # Shape: (batch_size, features)

        # Compute attention scores (energy)
        energy = torch.bmm(query, key.transpose(1,2))  # Shape: (batch_size, features//8, features//8)

        # Apply softmax to get the attention weights
        attention = self.softmax(energy)  # Shape: (batch_size, features//8, features//8)

        # Apply attention weights to value
        out = torch.bmm(attention, value)
        out = out.squeeze(1)  # Shape: (batch_size, features)

        return out


class MyModel(nn.Module):
    def __init__(self, num_classes=5, dropout_rate=0.55):  # Change _init_ to __init__
    ...
    def forward(self, x):
        ...
        x = self.backbone(x)
        x = self.self_attention(x)  # Apply Self Attention here
        x = torch.flatten(x, 1)
        x = self.fc(x)
        return x
...
class MyDualModel(nn.Module):
    def __init__(self, num_classes=5, dropout_rate=0.57):
        super().__init__()

        backbone = models.resnet18(pretrained=True)
        backbone.fc = nn.Identity()
        
        self_attention = selfattention(in_dim=512)
...
if __name__ == '__main__':
    ...
    # '''
    # Step 2: Load your custom pretrained weights
    custom_weights_path = "resnet18.pth" # Replace with your weight file path
    state_dict = torch.load(custom_weights_path, map_location="cpu")
    # Rename keys to match the expected structure
    new_state_dict = {}
    for key, value in state_dict.items():
      if not key.startswith("fc"):  # Exclude fc layer
    ...
    # Ensure the fc layer's parameters are trainable
