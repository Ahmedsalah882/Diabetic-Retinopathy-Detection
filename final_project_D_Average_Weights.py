# -*- coding: utf-8 -*-
"""Final_project_D_W_Final.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ctqrdILLqPcL2245MiXx0lIPz3wzqTgn
"""
...
class Clahe_Circle_Cropping:
    def __init__(self, apply_clahe=True, apply_circle_cropping=True):
        self.apply_clahe = apply_clahe
        self.apply_circle_cropping = apply_circle_cropping
    ...
    def preprocess(self, img):
        ...
        if self.apply_circle_cropping:
            # Circle cropping
            height, width = img.shape[:2]
            center = (width // 2, height // 2)
            # Use the smallest dimension as radius
            radius = min(center) 
...
transform_train = transforms.Compose([
    ...
    Clahe_Circle_Cropping(apply_clahe=True, apply_circle_cropping=True),
    transforms.ToTensor(),
...
#this is the one has been used during the training
...
class MyDualModeldense121(nn.Module):
    def __init__(self, num_classes=5, dropout_rate=0.7):
        ...
        self.backbone1 = copy.deepcopy(backbone)
        self.backbone2 = copy.deepcopy(backbone)
        # Initialize this to be dynamically calculated
        self.backbone1.classifier= copy.deepcopy( backbone.classifier)
        self.backbone2.classifier = copy.deepcopy( backbone.classifier)
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            for model, weight in zip(self.models, self.weights):
                outputs = model(x)  # Get logits or probabilities from the model
                if combined_outputs is None:
                    combined_outputs = weight * outputs
                else:
                    combined_outputs += weight * outputs  # Weighted sum of outputs

        # Determine the final predictions by taking the class with the highest score
        final_preds = torch.argmax(combined_outputs, dim=1)
        return final_preds

class MyDualMaxVotingModel(nn.Module):
    def __init__(self, models, num_classes=5):
        super().__init__()
        self.models = models  # List of individual models for dual input
        self.num_classes = num_classes
    ...
    def forward(self, images):
        ...
        # Perform max voting
        all_preds = torch.stack(all_preds, dim=1)  # Shape: [batch_size, num_models]
        majority_preds, _ = torch.mode(all_preds, dim=1)  # Majority voting for each sample
...
if __name__ == '__main__':
    ...
    mode1 = 'single'  # forward single image to the model each time
    #mode1 = 'dual'  # forward two images of the same eye to the model and fuse the features


    mode2 = 'single'  # forward single image to the model each time
    # mode2 = 'dual'  # forward two images of the same eye to the model and fuse the features


    mode3 = 'single'  # forward single image to the model each time
    #mode3 = 'dual'  # forward two images of the same eye to the model and fuse the feature

    mode4 = 'single'  # forward single image to the model each time
    #mode3 = 'dual'  # forward two images of the same eye to the model and fuse the feature
    ...
    if mode4 == 'single':
       EnsembleModel = WeightedAverageModel(modeLs,weights)
    else:
       EnsembleModel = MyDualMaxVotingModel(modeLs)

    print(f'model1:{model1}   model2:{model2}   model3:{model3}  \n')
    print(f'Pipeline Mode1:{mode1} Pipeline Mode2:{mode2} Pipeline Mode3:{mode3} ')
    ...
    ensemble_model = WeightedAverageModel([model1, model2, model3], weights)
    # Make predictions on testing set and save the prediction results
    evaluate_model(EnsembleModel, test_loader, device, test_only=True)